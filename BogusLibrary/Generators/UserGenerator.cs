...
public static class UserGenerator
{
    ...
    /// <summary>
    /// Generates a single <see cref="User"/> object with randomized data.
    /// </summary>
    /// <param name="random">
    /// A boolean value indicating whether to use non-deterministic randomization.
    /// If set to <c>false</c>, a fixed seed is used for deterministic results.
    /// </param>
    /// <returns>
    /// A single <see cref="User"/> object populated with realistic, randomized data.
    /// </returns>
    public static User CreateOne(bool random = false)
        => Create(1, random).First();
}