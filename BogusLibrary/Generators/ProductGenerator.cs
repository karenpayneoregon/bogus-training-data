...
public class ProductGenerator
{
    ...
    /// <summary>
    /// Generates a single <see cref="Categories"/> instance with associated products.
    /// </summary>
    /// <param name="random">
    /// Specifies whether the generation should be deterministic or random.
    /// <c>false</c> for deterministic generation (fixed seed, repeatable output),
    /// <c>true</c> for random generation (different results each run).
    /// </param>
    /// <returns>
    /// A single <see cref="Categories"/> instance with its <see cref="Categories.Products"/> collection populated.
    /// </returns>
